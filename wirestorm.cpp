...
const size_t HEADER_SIZE =
    8;  // MAGIC (1 byte) + PADDING (1 byte) + LENGTH (2 bytes)
...
uint16_t calculate_checksum_validation(uint8_t* header, uint8_t* data,
                                       uint16_t data_length);

// Global shared resources for inter-thread communication and client management
std::vector<int>
    destination_clients;   // List of active destination client sockets
std::mutex clients_mutex;  // Mutex to protect 'destination_clients' vector

std::queue<CTMPMessage>
    message_queue;       // Queue for messages from source to forwarder
std::mutex queue_mutex;  // Mutex to protect 'message_queue'
std::condition_variable
    queue_cv;  // Condition variable to signal new messages in queue
...
ssize_t write_n_bytes(int sockfd, const uint8_t* buffer, size_t n_bytes,
                      int timeout_sec) {
  size_t bytes_sent = 0;
  ...
  while (bytes_sent < n_bytes) {
    ...
    if (result == 0) {
      ...
      if (errno == EWOULDBLOCK || errno == EAGAIN) {
        ...
        // Other error
        std::cerr << "Error writing to socket " << sockfd << ": "
                  << strerror(errno) << std::endl;
        return -1;
      }
    }
...
void source_listener_thread() {
  ...
  // Set socket options to reuse address and port, preventing "Address already
  // in use" errors
  ...
  while (true) {
    ...
    while (true) {
      ...
      uint16_t padding =
          (static_cast<uint16_t>(header_buf[6]) << 8) | header_buf[7];
      ...
      if (magic != MAGIC_BYTE) {
        std::cerr << "Source listener: Invalid MAGIC byte (0x" << std::hex
                  << (int)magic << " received, expected 0x" << (int)MAGIC_BYTE
                  << "). Dropping message from " << client_ip << std::endl;
        continue;
      }
      ...
      if (length > MAX_PAYLOAD_SIZE) {
        std::cerr << "Source listener: Excessive LENGTH (" << length
                  ...
                  << " bytes). Dropping message from " << client_ip
                  << std::endl;
        continue;
      }
      ...
      if (bytes_read <= 0) {
        std::cerr << "Source listener: Source client " << client_ip
                  << " disconnected or read error/timeout during data payload."
                  << std::endl;
        break;
      }
      ...
      if (is_sensitive) {
        ...
        if (calculated_checksum != checksum) {
          std::cerr << "Source listener: Invalid CHECKSUM (received: 0x"
                    << std::hex << checksum << ", calculated: 0x"
                    << calculated_checksum
                    << "). Dropping sensitive message from " << client_ip
                    << std::endl;
          continue;
        }
      }
  ...
  close(server_fd);  // Close the server listener socket (though unreachable in
                     // current loop)
}
...
void destination_listener_thread() {
  ...
  while (true) {
    ...
    std::cout << "Destination listener: Client " << client_socket
...
void message_forwarder_thread() {
  ...
  while (true) {
    ...
    std::cout << "Forwarder: Got message of size "
              << message_to_send.data.size() << ", checking for clients..."
              << std::endl;
    ...
    {
      std::lock_guard<std::mutex> lock(clients_mutex);
      std::cout << "Forwarder: Found " << destination_clients.size()
                << " destination clients" << std::endl;

      for (int client_socket : destination_clients) {
        std::cout << "Forwarder: Sending to client socket " << client_socket
                  << std::endl;
        ...
        if (result == (ssize_t)message_to_send.data.size()) {
          ...
          std::cout << "Forwarder: Successfully sent " << result
                    << " bytes to client " << client_socket << std::endl;
        } else {
          std::cerr << "Forwarder: Failed to send to client " << client_socket
                    << " (sent " << result << " of "
                    << message_to_send.data.size() << " bytes)" << std::endl;
          close(client_socket);
        }
      }
    ...
    std::cout << "Forwarder: Message forwarded to " << active_clients.size()
              << " active clients." << std::endl;
  }
}
...
// Calculate checksum for validation (includes the actual checksum in the
// header)
uint16_t calculate_checksum_validation(uint8_t* header, uint8_t* data,
                                       uint16_t data_length) {
  uint32_t sum = 0;
